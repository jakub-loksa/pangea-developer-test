...
namespace DiffChecker.Api.Tests.Controllers.V1.DiffCheckerController
{
    ...
    public class SaveRightDiffTests : ApiTestBase
    {
        public SaveRightDiffTests(ApiTestApplicationFactory factory)
            : base(factory)
        {
            _factory.SetUpTest();
        }
        ...
        public async Task WhenInvalidBase64String_ShouldReturn400BadRequest()
        {
            ...
            var route = string.Format(RouteConstants.DiffRightRouteV1, "NOT_FOUND_DIFF");
        ...
        public async Task WhenDiffDoesNotExist_ShouldCreateNewDiff()
        {
            ...
            diff.LeftDiff.Should().BeNull();
            diff.RightDiff.Should().Be("Test Value");
        }
        ...
        public async Task WhenDiffExists_ShouldUpdateExistingDiff()
        {
            ...
            var route = string.Format(RouteConstants.DiffRightRouteV1, "SAME1");
            await _client.PostRaw<string>(route, requestBase64, HttpStatusCode.NoContent);

            using var context = GetContext();
            context.Diffs.Should().HaveCount(5, "It's the original count in the database.");
            var diff = context.Diffs.Single(x => x.Id == "SAME1");
            diff.LeftDiff.Should().Be("ABC123", "It's the original value stored in the database");
            diff.RightDiff.Should().Be("CHANGED_VALUE");
        }
    }
}
